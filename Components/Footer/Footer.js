...
const Footer = () => {
	// Add state variables to track scroll and chat bubble visibility
	const [scrolledToBottom, setScrolledToBottom] = useState(false);
	const [showChatBubble, setShowChatBubble] = useState(false);

	useEffect(() => {
		const handleScroll = () => {
		  // Calculate the scroll position and the total scroll height
		  ...
		  if (scrollPosition + viewportHeight >= totalScrollHeight) {
			...
			// Wait for 3 seconds and then show the chat bubble
			setTimeout(() => {
